...
因为这时候除了排序过程外，在排序完成后，还要根据id去原表取值。由于语句是limit
1000，因此会多读1000行。
...
[图8 引入(city,name)联合索引后，查询语句的执行计划]{.reference}
...
根据上面说的规则，"既然读了数据，就会判断"， 因此在这时候，select \*
